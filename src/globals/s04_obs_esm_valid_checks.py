...
# Scientific Data Descriptor paper). Some outputs are clipped to their respective domains and all are provided on a
# standard leap-year calendar.
...
from src.config.data_sets import (CHIRPS_CONFIG, ERA5_LAND_CONFIG, MODEL_CONFIGS, get_abbreviation, get_long_variable,
                                  get_unit, get_merged_path_filename)
from src.utils.path_helper import create_folder, get_files_by_pattern, compress
from src.config.data_catalog import (get_global_projections, domains_extent_file, get_compressed_folder,
                                     get_merged_folder, get_ref_paths)
...
def main(update_config: DateUpdateArgs, realization_gcm, scenarios_gcm, output_dir=domains_extent_file):
    ...
    main_obs_merged(path_chirps_compressed, min_val_chirps, max_val_chirps, excluded_variables, config)
    lines = []
    for reg_sel in cpd_region_list.keys():
        ...
        for scenario in scenarios_gcm:
            ...
            lines.append(block)
        main_obs_domain(update_config, reg_sel, lon_min, lon_max, lat_min, lat_max)
        lines.append("\n")
...
# main observations and climate model outputs
def main_obs_merged(path_chirps_compressed, min_val_chirps, max_val_chirps, excluded_variables,
                    update_config: DateUpdateArgs):
    ...
    for year_sel in list(range(update_config.year_start, update_config.year_end + 1)):
        if year_sel > 1980:
            (path_chirps_merged_year, path_chirps_merged_year_box, path_chirps_merged_year_time,
             path_chirps_merged_year_inter,
             path_chirps_merged_year_global, path_chrips_merged_year_final) = get_merged_paths(
                CHIRPS_CONFIG, year_sel,
                variable="total_precipitation")
            check_files_year_obs(input_path=path_chirps_merged_year_global, year=year_sel,
                                 variable="total_precipitation",
        ...
        for variable_sel in variables_era5land:
            ...
            (path_era5land_merged_year, path_era5land_merged_year_box,
             path_era5land_merged_year_time, path_era5land_merged_year_inter, path_era5land_merged_year_global,
             path_era5land_merged_year_final) = get_merged_paths(ERA5_LAND_CONFIG, year_sel, variable_sel)
            check_files_year_obs(input_path=path_era5land_merged_year_global, year=year_sel,
...
def main_obs_domain(domain, lon_min, lon_max, lat_min, lat_max, path_chirps_compressed,
                    min_val_chirps, max_val_chirps, excluded_variables, update_config: DateUpdateArgs):
    if update_config.year_start < 1950:
        raise AttributeError("Minimum year is 1950.")
    for year_sel in list(range(update_config.year_start, update_config.year_end + 1)):
        if year_sel > 1980:
            path_chirps_ref, path_chirps_ref_year = get_ref_paths(CHIRPS_CONFIG, year_sel,
                                                                  variable="total_precipitation")
            check_files_year_obs_domain(continent=domain, lon_min=lon_min, lon_max=lon_max, lat_min=lat_min,
                                        lat_max=lat_max, input_path=path_chirps_ref, year=year_sel,
                                        variable="total_precipitation",
                                        source="CHIRPS", path_base=path_chirps_compressed, version="version 3",
                                        min_val=min_val_chirps, max_val=max_val_chirps,
                                        link="https://www.chc.ucsb.edu/data/chirps3", start_year="1981")
        ...
        for variable_sel in variables_era5land:
            ...
            check_files_year_obs_domain(continent=domain, lon_min=lon_min, lon_max=lon_max, lat_min=lat_min,
                                        ...
                                        variable=variable_sel,
                                        source="ERA5-Land", path_base=path_era5land_compressed,
                                        version="extracted 2025",
                                        min_val=min_val_era5land, max_val=max_val_era5land,
                                        link="https://cds.climate.copernicus.eu/datasets/reanalysis-era5-land?tab=overview",
                                        start_year="1950")
...
# checks
def check_files_year_obs(input_path, year,
    ...
    ds_spatial = ds.rio.set_spatial_dims(x_dim="lon", y_dim="lat", inplace=True)
    data_spatial = ds_spatial.rio.write_crs(crs_reference)
    data_clipped, correction = clip_outliers_check(data_spatial, var, min_val, max_val)
    data_clipped = delete_coords_var(data_clipped)
    pattern = "{}_{}_compressed_native.nc".format(variable, year)
    path_file = os.path.join(path_base, pattern)
    variable_long_extended = get_long_variable(variable)
    conventions_obs(data_clipped, var, variable, variable_long_extended, unit, start_year, source, version, link)
...
def check_files_year_obs_domain(continent, lon_min, lon_max, lat_min, lat_max, input_path, year,
                                variable, path_base, source, version, link, min_val, max_val, start_year="1950"):
