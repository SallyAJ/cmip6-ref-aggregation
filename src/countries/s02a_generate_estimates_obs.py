...
from src.utils.pop_helper import prepare_pop, _clip_pop_to_country, get_pop_for_year, get_decade, weights_pop, load_decadal_populations
from src.utils.data_helper import read_in_xarray_var_crs, read_in_time
...
def main(domain, country_code, period_config, path_chirps, variables_era5land = variables_era5land_list):
    ...
    ds_variables["total_precipitation"] = prepare_continent_file(path_chirps, domain, "total_precipitation", period_config,
...
def prepare_continent_file(path_source, continent, variable, period,  shape_ad0, var_abb, crs=crs_reference_global):
    datasets = []
    for year in period:
    ...
    ds_merged.rio.write_crs(crs, inplace=True)
    ...
    for admin_unit in admin_unit_levels:
        ...
        if shape_country is not None:
            ...
            for index in indices_list:
                run_country_obs(country_code, SEDAC_CONFIG, index, shape_country_ad0, shape_country, admin_names, ds_variables,
                                variables_era5land,
                                domain,
                                period_config)

def run_country_obs(country_code, sedac_config, index, shape_country_ad0, shape_country, admin_names, ds_variables, variables_era5land_list,
                        domain,
                        period_config):
...
def pq_file(polygon_gdf, cleaned_admin_unit_name, country, period, ds_variables, pop_grids_ssp, variables_era5land, reg_sel):
...
def obs_avg(ds_variables, polygon_gdf, cleaned_admin_unit_name, pop_grids, var_long):
    ...
    results=[]
    mean_values_pop, mean_values_area, mean_values_pop_dynamic, num_pixels_nc = combine_years_obs(var_long, ds_variables, pop_grids, polygon_gdf)
    results.append({
...
if __name__ == "__main__":
    ...
    main(domain_sel, country_code_sel, period_config_sel, path_chirps=path_chirps_load, variables_era5land=variables_era5land_list)
